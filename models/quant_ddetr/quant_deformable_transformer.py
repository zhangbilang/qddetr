...
from .lsq_plus import LinearLSQ
from .attention_layer import QuantMultiheadAttention
from .ms_attention_layer import QuantMS_DAttention

from torchvision.ops import RoIAlign
from .quant_sapm_separableconv import SAPM, MLP, convert_to_rois
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8, n_bit=4, 
                 num_encoder_layers=6, num_decoder_layers=6, dim_feedforward=1024, dropout=0.1,
                 activation="relu", return_intermediate_dec=False,
                 num_feature_levels=4, dec_n_points=4,  enc_n_points=4,
                 two_stage=False, two_stage_num_proposals=300, use_sapm=False):
        super().__init__()
        ...
        encoder_layer = DeformableTransformerEncoderLayer(d_model, n_bit, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, enc_n_points)
        self.encoder = DeformableTransformerEncoder(encoder_layer, num_encoder_layers)

        decoder_layer = DeformableTransformerDecoderLayer(d_model, n_bit, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, dec_n_points)
        self.decoder = DeformableTransformerDecoder(decoder_layer, num_decoder_layers, return_intermediate_dec, use_sapm=use_sapm)
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
                 d_model=256, n_bit=4, d_ffn=1024,
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, n_bit=4, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4):
        super().__init__()
        ...
        # self attention
        self.self_attn = QuantMultiheadAttention(d_model, n_heads, n_bit=n_bit, dropout=dropout)
        self.dropout2 = nn.Dropout(dropout)
        self.norm2 = nn.LayerNorm(d_model)
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, use_sapm=False):
        super().__init__()
